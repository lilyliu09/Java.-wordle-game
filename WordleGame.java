...
public class WordleGame {
    ...
    public WordleGame(String secretWord)
    {
        ...
        this.secretWord = secretWord;//assign a value from a parameter"secretWord" into a field "secretWord"
    }
    ...
    public boolean isInSecretWord(char letter)
    {
        for ( int i = 0; i < secretWord.length();i++) {
            if (letter == secretWord.charAt(i)) {
                return true;//if a letter appear any location of secretWord
            }
        }   
        return false; 
    }
    ...
    public String guessWord(String wordToGuess)
    {
        ...
        for (int i = 0; i < secretWord.length(); i++) {
            if (wordToGuess.charAt(i) == secretWord.charAt(i)) {
                result += "*";// if the letter of guess word appear and  in the same location with secret word
            } 
            else {
                // else means the "if" above is false, there is no location letter in guess word same with related location letter in secret word
                if(isInSecretWord(wordToGuess.charAt(i))) {
                    result += ".";//else if means have same letter but not in correct location
                } 
                else {
                    result += "_";// else else the letter not appear in the secret word
                }
            }
        }
        return result;
