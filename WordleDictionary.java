...
public class WordleDictionary
{
    ...
    public ArrayList<String> getWordsWithLength(int length)
    {
        ...
        for (String s : words) {
            if (s.length() == length) {
                newList.add(s);// if any word in the list words have the same length with getWordsWithLength 
                              //it will be addded in the new ArrayList
        }
        }
        return newList;    
        }
    ...
    {    
       ...
       int index = r.nextInt(getWordsWithLength(length).size());//method call with getWordWithLegnth
       return getWordsWithLength(length).get(index);//get the random word form the new ArrayLsit
    }
}
